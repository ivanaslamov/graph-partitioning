...
int refine( double *assignment, double *merges, int merges_rows, double *w, int m, int n, int w_order, int max_node, int cutstep )
{
  // variables
    ...
    for(map<int, set<int> >::iterator it=partitions[i].begin(); it!=partitions[i].end(); ++it)
    {
      ...
      if(i != j)
      {
        for (set<int>::iterator itt=it->second.begin(); itt!=it->second.end(); ++itt)
        {
          ...
          double merged_score = ( inner[j] + 2 * edge_part[u][j] ) / ( degree[j] + vertex_degree[u] );

          double remain_score = abs(degree[i] - vertex_degree[u]) < epsilon ? 0 : ( inner[i] - 2 * edge_part[u][i] ) / ( degree[i] - vertex_degree[u] );

          double old_score    = inner[i] / degree[i] + inner[j] / degree[j];

          double cost = merged_score + remain_score - old_score;

          if( max_cost < cost || node == -1 )
          {
            node = u;
            max_cost = cost;
          }
...
void mexFunction( int nlhs, mxArray *plhs[], int nrhs, const mxArray*prhs[] ) 
{
  ...
  int cutstep = (int) mxGetScalar(prhs[2]);
  ...
  refine(parts,merges,merges_m,w,m,n,1,max_node,cutstep);
}
