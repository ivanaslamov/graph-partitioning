...
int agglomerate( double *mp, double *w, int m, int n, int w_order, int max_node, int capacity )
{
  ...
  for(int i=0;i<m;i++)
  {
    ...
    double weight = w_order ? w[i+2*m] : w[n*i+2];

    if( so != si )
    {
      edges[so].insert( std::pair<int,double>(si,weight) );
      edges[si].insert( std::pair<int,double>(so,weight) );  
    }
  }
  ...
  for(int i=0;i<indexes.size();i++)
  {
    ...
    if( edges[ indexes[i] ].size() > 0 )
    {
    	...
    	for (std::map<int,double>::iterator it=edges[ indexes[i] ].begin(); it!=edges[ indexes[i] ].end(); ++it)
  		{       
  			double dcost = it->second - degree[indexes[i]] * degree[it->first] / (2*total);
...
t1=clock();
  ...
  for(k=0;k<max_node-1 && tree;k++)
  {
    ...
    std::map<int,double>::iterator it;
  	...
  	for (it=edges[n].begin(); it!=edges[n].end(); it++)
    {
    ...
    for (it=edges[m].begin(); it!=edges[m].end(); it++)
    {
    	if( connections[ it->first ].partition == m || connections[ it->first ].partition == n )
    	{
    		connections[ it->first ].partition = -1;
    ...
    connections[n].cost      = -INFINITY;
  	...
  	{
  		double dcost = it->second - degree[ n ] * degree[ it->first ] / (2.0 * total);
